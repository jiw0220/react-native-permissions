        ...
        Permissions.checkMultiplePermissions(types)
            .then(status => {
                if (this.state.isAlways) {
                    return Permissions.getPermissionStatus('location', 'always').then(location => ({...status, location}))
                }
                return status
            })
        ...
        return (
            <View style={styles.container}>
                ...
                <Text style={styles['footer_' + Platform.OS]}>
                    Note: microphone permissions may not work on iOS simulator. Also, toggling permissions from the settings menu may cause the app to crash.
                    This is normal on iOS. Google "ios crash permission change"
                </Text>
            </View>
        );
    }
